...
visualization_msgs::Marker markerBuilder(int id,XYZ loc,XYZ scale,XYZ min,XYZ max, float headding,int size){//used to create markers to be placed in a marker array. performs some error adjustment
	...
	if(abs(xLoc)<=2.00){//close to center of x
		locMultiplier=1.25;
		if(yLoc<-2.2){
			yLoc=yLoc-.65;
		}else if(yLoc>2.2){
	...
	}else if (abs(xLoc)<=1.0){//very close to center of x
		locMultiplier=2;
		if(yLoc<0){
			yLoc=yLoc-1.9;
		}else{
	...
	}else if (abs(yLoc)<=2){//close to center of y
		locMultiplier=1.3;
		if(xLoc<0){
			xLoc=xLoc-.25;
		}else{
	...
	for(int i=0; i<6;i++){
		...
		if(yScaleResult<=yScale){
			sizeResult=sizeResult+(sizeResult/2);
		}
		if((yLoc+(yScaleResult/2))<max.y){
			yLoc=yLoc+.6;
		}
		if((yLoc-(yScaleResult/2))>min.y){
			yLoc=yLoc-.6;
		}
