		...
		const raw_measure_t& laserReturn, const pandar_pointcloud::PandarLaserCorrection& correction)
{
    ...
    float x,y,z;
    x = static_cast<float> (xyDistance * sin_azimuth - correction.horizontalOffsetCorrection * cos_azimuth);
    y = static_cast<float> (xyDistance * cos_azimuth + correction.horizontalOffsetCorrection * sin_azimuth);
    z = static_cast<float> (distanceM * correction.sinVertCorrection + correction.verticalOffsetCorrection);

    /** Use standard ROS coordinate system (right-hand rule) */  //This makes the lidar coordinate is the same as the car coordinate
    float x_coord = -y;
    float y_coord = x;
    float z_coord = z;
